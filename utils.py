...
class FocalLoss_poly(nn.Module):
    def __init__(self,alpha,gamma,epsilon,num_classes=2,size_average=True):
    ...
    def forward(self, pred, target):
        ...
        pred = torch.stack((pred, 1 - pred)).view(pred.shape[0], 2, *pred.shape[1:])
        p=pred
        target = target.long()
...
class FocalLoss(nn.Module):
    def __init__(self,alpha,gamma,num_classes=2,size_average=True):
...
class AttentionModule(nn.Module):
    def __init__(self, input_size: int, hidden_size: int):
        ...
        self.lstm = nn.LSTM(input_size=self.input_size, hidden_size=self.hidden_size, batch_first=False, num_layers=1)
    ...
    def forward(self, input_tensor):
        ...
        for t in range(seq_len):
            ...
            attn_output = torch.mul(attn_combine_fc.cuda(), cur_inp_layer_res.cuda()) #attn_output = torch.mul(attn_combine_fc, cur_inp_layer_res)
            attn_output += attn_output

        output_tensor, hidden_tensor = self.lstm(attn_output, hidden_tensor)
